...
using AllGamesCoinLib.Auxiliary;
using AllGamesCoinLib.Services.Coins.Base;
using AllGamesCoinLib.Services.Coins.AllGamesCoin;
    ...
    public partial class CoinService
    {
        ...
        public class CoinParameters
        {
                ...
                short rpcRequestTimeoutInSeconds)
            {
                ...
                else if (coinService is AllGamesCoinService)
                {
                    if (!IgnoreConfigFiles)
                    {
                        DaemonUrl = ConfigurationManager.AppSettings.Get("AllGamesCoin_DaemonUrl");
                        DaemonUrlTestnet = ConfigurationManager.AppSettings.Get("AllGamesCoin_DaemonUrl_Testnet");
                        RpcUsername = ConfigurationManager.AppSettings.Get("AllGamesCoin_RpcUsername");
                        RpcPassword = ConfigurationManager.AppSettings.Get("AllGamesCoin_RpcPassword");
                        WalletPassword = ConfigurationManager.AppSettings.Get("AllGamesCoin_WalletPassword");
                    }

                    CoinShortName = "XAGC";
                    CoinLongName = "AllGamesCoin";
                    IsoCurrencyCode = "XAGC";
